...
```bash
git remote add origin git@github.com:eliavw/residual-anomaly-detector.git
git remote -v
...
```bash
git remote add origin git@github.com:eliavw/residual-anomaly-detector.git; git remote -v; git push origin master
```
...
```bash
conda env create -f dependencies-deploy.yaml -n residual-anomaly-detector
```

To additionally add the packages which are relevant for the development phase, do:

```bash
conda activate residual-anomaly-detector
conda env update -n residual-anomaly-detector -f dependencies-develop.yaml
```
...
```bash
conda activate residual-anomaly-detector
python -m ipykernel install --user --name residual-anomaly-detector --display-name "residual-anomaly-detector"
```
...
1. Go to the [Travis](https://travis-ci.com/eliavw/residual-anomaly-detector) page of this repo.
2. See if it ran.
...
and your site should be online at; [https://eliavw.github.io/residual-anomaly-detector/](https://eliavw.github.io/residual-anomaly-detector/). 
...
> Anomaly Detector on the basic assumption that high residuals of surrogate models can be indicative of anomalous behavior, cf. https://eliavw.github.io/residual-anomaly-detector
